...
(define data-frame%
  (class object%
    ...
    ;; Helper function for `shallow-copy`, which see
    (define/public-final (%set-internal-state ds ls p)
      (set! data-series ds)
      (set! lazy-series ls)
      (set! properties p))

    ;; Create a copy of this data frame.  The internal data frame data
    ;; structures are copied, but the same series objects and properties are
    ;; referenced.  Calling this function will materialize all lazy series
    ;; first.
    (define/public (shallow-copy)
      ;; Materialize the lazy series now, we cannot copy them as they are
      ;; closures over the original object.  Note that we cannot use
      ;; `in-hash-keys' here, as lazy-series will be modified while we run the
      ;; loop.
      (for ([s (hash-keys lazy-series)])
        (get-series s))
      (define df-copy (new data-frame%))
      (send df-copy %set-internal-state
            (hash-copy data-series)
            (make-hash)
    ...
    (define/public (del-property key)
    ...
    ;; Remove series NAME from the data frame.  NOTE: there might be
    ;; implications of deleting series when they are referenced by lazy
    ;; derived series...
    (define/public (del-series name)
      (hash-remove! data-series name)
      (hash-remove! lazy-series name))
